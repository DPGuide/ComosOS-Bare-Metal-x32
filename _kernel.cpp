/* COSMOS OS v38.5 - STANDARD C++ TRANSLATION */
/* ORIGINAL ARCHITECT: TOBIAS SCHNEIDER | TARGET: BARE METAL */

#include <stdint.h>
/* COSMOS OS v38.5 - MULTIBOOT 1 KOMPATIBEL */
// Multiboot 1 Struktur (korrekt!)
struct multiboot_info {
    ...
    // ELF sections (wenn flags[5])
    uint32_t num;
    uint32_t size;
    uint32_t addr;
    uint32_t shndx;
    
    // Memory map (wenn flags[6])
    ...
    // Config table (wenn flags[8])
    uint32_t config_table;
    
    // Bootloader name (wenn flags[9])
    uint32_t boot_loader_name;
    
    // APM table (wenn flags[10])
    uint32_t apm_table;
    
    // VBE (wenn flags[11]) - DAS IST WICHTIG!
    ...
    // Framebuffer (wenn flags[12]) - Multiboot 1.0+
    uint64_t framebuffer_addr;
    uint32_t framebuffer_pitch;
    uint32_t framebuffer_width;
    uint32_t framebuffer_height;
    uint8_t  framebuffer_bpp;
    uint8_t  framebuffer_type;  // 0=indexed, 1=RGB, 2=EGAText
...
// Deine existierenden Strukturen bleiben gleich...
struct PhysicalDrive { bool present; char model[41]; uint32_t size_mb; uint16_t base_port; bool is_slave; int type; } __attribute__((packed));

// ==========================================
// 1. DATA STRUCTURES
// ==========================================
struct PhysicalDrive { bool present; char model[41]; uint32_t size_mb; uint16_t base_port; bool is_slave; int type; } __attribute__((packed));
struct Partition { uint8_t status; uint8_t type; uint32_t start_lba; uint32_t size; } __attribute__((packed));
struct FileEntry { char name[32]; uint32_t sector_offset; bool is_folder; uint32_t size; char date[16]; bool exists; uint8_t parent_idx; uint8_t padding[5]; } __attribute__((packed));
...
struct SpiritTarget { char ip[20]; int hits; bool critical; int x_off, y_off; };

// NET STACK
struct EthernetFrame { uint8_t dest_mac[6]; uint8_t src_mac[6]; uint16_t type; } __attribute__((packed)); 
struct IPHeader { uint8_t ver_ihl; uint8_t tos; uint16_t len; uint16_t id; uint16_t frag; uint8_t ttl; uint8_t proto; uint16_t chk; uint32_t src; uint32_t dst; } __attribute__((packed));  
struct UDPHeader { uint16_t src; uint16_t dst; uint16_t len; uint16_t chk; } __attribute__((packed));
struct TCPHeader { uint16_t src, dst; uint32_t seq, ack; uint8_t off, flg; uint16_t win, chk, urg; } __attribute__((packed));
...
int screen_h = 600; 
uint32_t* fb = nullptr; 
uint32_t* bb = (uint32_t*)0x2000000; 
int pitch = 3200;  // 800*4 Default
...
bool mouse_left=false, mouse_just_pressed=false; uint8_t last_key = 0; bool key_new = false; bool key_shift = false;

// WINDOWS
int win_z[11] = {0, 1, 2, 4, 3, 6, 5, 7, 8, 9, 10}; 
int drag_win = -1; int drag_off_x = 0; int drag_off_y = 0; int resize_win = -1; bool z_blocked = false;
...
// HOLYSPIRIT GLOBALS
SpiritTarget spirit_targets[10]; int spirit_count = 0;
bool spirit_active = false; int spirit_timer = 0;
char spirit_logs[6][40];
...
FileEntry file_table[8]; int current_open_file = -1; int drive_status = 0; uint8_t hdd_buf[512]; 
uint8_t current_path_id = 255; int current_folder_view_idx = -1; 
int dsk_view = 0; int dsk_selection = 0; int file_selection = -1; int active_drive_idx = -1; int input_cooldown = 0;

// DRIVERS
uint32_t rtl_io_base = 0; uint32_t intel_mem_base = 0; uint32_t usb_io_base = 0;
...
uint8_t* rx_buffer_rtl = (uint8_t*)0x600000; int rx_idx_rtl = 0;
e1000_rx_desc rx_desc_intel[32]; uint8_t* rx_bufs_intel[32]; int rx_cur_intel = 0;
...
Window windows[11]; Planet planets[6]; Star stars[200]; int senses_tab = 0;

// ==========================================
// 3. LOW LEVEL (ASM) & MATH
// ==========================================
inline uint8_t inb(uint16_t p) { uint8_t r; asm volatile("inb %1, %0":"=a"(r):"Nd"(p)); return r; }
inline void outb(uint16_t p, uint8_t v) { asm volatile("outb %0, %1"::"a"(v),"Nd"(p)); }
inline void outw(uint16_t p, uint16_t v) { asm volatile("outw %0, %1"::"a"(v),"Nd"(p)); }
inline uint32_t inl(uint16_t p) { uint32_t r; asm volatile("inl %1, %0":"=a"(r):"Nd"(p)); return r; }
inline void outl(uint16_t p, uint32_t v) { asm volatile("outl %0, %1"::"a"(v),"Nd"(p)); }
...
uint32_t mmio_read32(uint32_t addr) { return *(volatile uint32_t*)(uintptr_t)addr; }
void mmio_write32(uint32_t addr, uint32_t val) { *(volatile uint32_t*)(uintptr_t)addr = val; }
...
char get_ascii(uint8_t sc) {
    if(sc > 58) return 0;
    static const char k_low[] = {0,27,'1','2','3','4','5','6','7','8','9','0','-','=','\b','\t','q','w','e','r','t','y','u','i','o','p','[',']','\n',0,'a','s','d','f','g','h','j','k','l',';','\'','`',0,'\\','z','x','c','v','b','n','m',',','.','/',0,'*',0,' '};
    static const char k_up[]  = {0,27,'!','@','#','$','%','^','&','*','(',')','_','+','\b','\t','Q','W','E','R','T','Y','U','I','O','P','{','}','\n',0,'A','S','D','F','G','H','J','K','L',':','"','~',0,'|','Z','X','C','V','B','N','M','<','>','?',0,'*',0,' '};
    return key_shift ? k_up[sc] : k_low[sc];
}
bool is_over_rect(int mx, int my, int x, int y, int w, int h) { int m = touch_mode ? 30 : 0; return (mx >= x-m && mx <= x+w+m && my >= y-m && my <= y+h+m); }
bool is_over(int mx, int my, int ox, int oy, int r) { int m = touch_mode ? 20 : 0; return (mx-ox)*(mx-ox) + (my-oy)*(my-oy) < (r+m)*(r+m); }
...
void Swap() { if(shake_timer > 0) { sx_off = (random()%10)-5; sy_off = (random()%10)-5; shake_timer--; } else { sx_off=0; sy_off=0; } for(int y=0; y<600; y++) { int ty = y + sy_off; if(ty < 0 || ty >= 600) continue; uint32_t* d=(uint32_t*)((uint8_t*)fb+ty*pitch); uint32_t* s=&bb[y*800]; for(int x=0; x<800; x++) { int tx = x + sx_off; if(tx >= 0 && tx < 800) d[tx] = s[x]; } } }

void DrawRoundWindow(int cx_p, int cy_p, int r, uint32_t color) { int r2=r*r; for(int y=-r;y<=r;y++) for(int x=-r;x<=r;x++) if(x*x+y*y<=r2) Put(cx_p+x, cy_p+y, color); }
void DrawIcon(int x, int y, uint32_t col) { for(int i=0; i<30; i++) for(int j=0; j<40; j++) Put(x+i, y+j, col); for(int i=5; i<25; i++) for(int j=5; j<15; j++) Put(x+i, y+j, 0xFFFFFF); }
void DrawCentrifugalVortex(int v_cx, int v_cy, int exp) { if (exp <= 5) return; int rot = (256 - (frame * 85) % 256) % 256; uint32_t dust_col = 0x222222; int max_sync_dist = (280 * exp) / 320; for (int r = 0; r < 9; r++) { int dist = ( (40 + r * 35) * exp ) / 320; if(dist > max_sync_dist) continue; for (int i = 0; i < 256; i += 4) { int ang = (i + rot) % 256; Put(v_cx + (Cos(ang)*dist)/84, v_cy + (Sin(ang)*dist*3/4)/84, dust_col + (random()%0x353535)); } } int axes[] = {0, 64, 128, 192}; for(int a=0; a<4; a++) { int axis_ang = (axes[a] + rot) % 256; for(int d=10; d < max_sync_dist; d+=3) { PutAlpha(v_cx + (Cos(axis_ang)*d*4)/336, v_cy + (Sin(axis_ang)*d*3)/336, 0x707070 + (random()%0x505050)); } } }
...
void DrawChar(int x, int y, char c, uint32_t col, bool bold) { 
    ...
    static const uint8_t f_l[] = {  };
    ...
    const uint8_t* ptr = 0;
    if(c >= 'A' && c <= 'Z') ptr = &f_u[(c-'A')*5]; else if(c >= 'a' && c <= 'z') ptr = &f_l[(c-'a')*5]; else if(c >= '0' && c <= '9') ptr = &f_n[(c-'0')*5];
    else if(c == ':') { static const uint8_t s[]={0,0x36,0x36,0,0}; ptr=s; } else if(c == '.') { static const uint8_t s[]={0,0x60,0x60,0,0}; ptr=s; } else if(c == '-') { static const uint8_t s[]={0x08,0x08,0x08,0x08,0x08}; ptr=s; } else if(c == '!') { static const uint8_t s[]={0,0,0x7D,0,0}; ptr=s; } else if(c == '_') { static const uint8_t s[]={0x40,0x40,0x40,0x40,0x40}; ptr=s; } else if(c == '(') { static const uint8_t s[]={0x1C,0x22,0x41,0,0}; ptr=s; } else if(c == ')') { static const uint8_t s[]={0,0,0x41,0x22,0x1C}; ptr=s; } else if(c == '/') { static const uint8_t s[]={0x20,0x10,0x08,0x04,0x02}; ptr=s; } else if(c == '=') { static const uint8_t s[]={0x14,0x14,0x14,0x14,0x14}; ptr=s; } else if(c == '?') { static const uint8_t s[]={0x20,0x40,0x45,0x48,0x30}; ptr=s; } else if(c == '"') { static const uint8_t s[]={0,0x03,0,0x03,0}; ptr=s; } else if(c == ',') { static const uint8_t s[]={0,0x50,0x30,0,0}; ptr=s; } else if(c == '|') { static const uint8_t s[]={0x7F,0,0,0,0}; ptr=s; }
    if(!ptr) return; for(int m=0;m<5;m++){ uint8_t l=ptr[m]; for(int n=0;n<7;n++) if((l>>n)&1) { Put(x+m,y+n,col); if(bold) Put(x+m+1,y+n,col); } } 
}
...
void DrawFace(int x, int y, int mx, int my) { DrawRoundWindow(x, y, 60, 0x555555); int ex1=x-20; int ey=y-10; int ex2=x+20; int ox=(mx-x)/20; if(ox>5)ox=5; if(ox<-5)ox=-5; int oy=(my-y)/20; if(oy>5)oy=5; if(oy<-5)oy=-5; DrawRoundedRect(ex1-8, ey-5, 16, 10, 2, 0xFFFFFF); DrawRoundedRect(ex2-8, ey-5, 16, 10, 2, 0xFFFFFF); Put(ex1+ox, ey+oy, 0); Put(ex1+ox+1, ey+oy, 0); Put(ex2+ox, ey+oy, 0); Put(ex2+ox+1, ey+oy, 0); DrawRoundedRect(x-15, y+25, 30, 2, 0, 0); }
void DrawWaveform(int x, int y, int w, int f) { for(int i=0; i<w; i+=2) { int h = Sin(i*5 + f*10) / 4; if(random()%10 > 7) h += (random()%10)-5; Put(x+i, y+20+h, 0); } }

// BOOT LOGO
void boot_logo() {
    uint32_t g=0xFFD700; int c=400, y=200;
    TextC(c,y,"#####################################",g,true);
    TextC(c,y+20," SCHNEIDER SYSTEM v38.5 (ULTIMATE) ",0xFFFFFF,true);
    TextC(c,y+40," ID-HASHING: ACTIVE | NET: RX/TX ",0xAAAAAA,false);
    TextC(c,y+80," >> SYSTEM READY << ",g,true);
    TextC(c,y+100,"#####################################",g,true);
}

// HOLYSPIRIT RENDERER
void draw_spirit_radar(int wx, int wy, int ww, int wh) {
    DrawRoundedRect(wx+10, wy+40, ww-20, wh-60, 5, 0x050010);
    int cx = wx + ww/2; int cy = wy + 40 + (wh-60)/2;
    for(int r=20; r<120; r+=30) for(int a=0; a<256; a+=16) Put(cx + (Cos(a)*r)/84, cy + (Sin(a)*r*3/4)/84, 0x330033);
    int ang = (frame*4)%256;
    for(int r=0; r<100; r+=2) Put(cx + (Cos(ang)*r)/84, cy + (Sin(ang)*r*3/4)/84, 0x00FF00);
    for(int i=0; i<spirit_count; i++) {
        int tx = cx + spirit_targets[i].x_off; int ty = cy + spirit_targets[i].y_off;
        DrawRoundWindow(tx, ty, 3, spirit_targets[i].critical ? 0xFF0000 : 0x00FFFF);
        if(is_over(mouse_x, mouse_y, tx, ty, 5)) Text(tx+5, ty, spirit_targets[i].ip, 0xFFFFFF, true);
    }
    for(int i=0; i<6; i++) Text(wx+20, wy+wh-30-(i*12), spirit_logs[i], 0x00FF00, false);
}
...
void play_earthquake(int duration) { shake_timer = duration; while(shake_timer > 0) { play_freq(40 + (random() % 60)); Swap(); for(volatile int d=0; d<40000; d++); } play_freq(0); }
...
void read_rtc() { outb(0x70, 4); rtc_h = bcd2bin(inb(0x71)); outb(0x70, 2); rtc_m = bcd2bin(inb(0x71)); outb(0x70, 7); rtc_day = bcd2bin(inb(0x71)); outb(0x70, 8); rtc_mon = bcd2bin(inb(0x71)); outb(0x70, 9); rtc_year = bcd2bin(inb(0x71)); rtc_h = (rtc_h + 1) % 24; }
void get_cpu_brand() { uint32_t a,b,c,d; asm volatile("cpuid":"=a"(a):"a"(0x80000000)); if(a<0x80000004){str_cpy(cpu_brand,"GENERIC");return;} char* s=cpu_brand; for(uint32_t i=0x80000002;i<=0x80000004;i++){asm volatile("cpuid":"=a"(a),"=b"(b),"=c"(c),"=d"(d):"a"(i));*(uint32_t*)s=a;s+=4;*(uint32_t*)s=b;s+=4;*(uint32_t*)s=c;s+=4;*(uint32_t*)s=d;s+=4;} cpu_brand[48]=0; }
...
void handle_input() { 
    ...
    if(st & 1) { 
        ...
        if(st & 0x20) { 
            ...
            if(d == 0x2A || d == 0x36) key_shift = true; else if(d == 0xAA || d == 0xB6) key_shift = false; else if(!(d & 0x80)) { last_key = d; key_new = true; } if(active_mode == 1) active_mode = 2; 
        } 
    } 
}
...
// --- NETWORK STACK ---
void net_raw(void* d, uint32_t l) {
    if(!rtl_io_base && !intel_mem_base) return;
    if(intel_mem_base > 0) { return; } 
    if(rtl_io_base > 0) {
        for(uint32_t i=0;i<l;i++) tx_buffer[i]=((uint8_t*)d)[i];
        outl(rtl_io_base+0x20+(tx_cur*4),(uint32_t)tx_buffer); outl(rtl_io_base+0x10+(tx_cur*4),l);
        tx_cur=(tx_cur+1)%4;
    }
}

void net_ip(uint32_t dst, void* p_data, uint16_t p_len, uint8_t proto) {
    uint8_t b[1514]; EthernetFrame* e=(EthernetFrame*)b; IPHeader* i=(IPHeader*)(b+14);
    for(int k=0;k<6;k++){e->dest_mac[k]=0xFF; e->src_mac[k]=mac_addr[k];} e->type=hs(0x0800);
    ...
    for(int k=0;k<p_len;k++) b[34+k]=((uint8_t*)p_data)[k];
    net_raw(b, 34+p_len);
}

void send_udp(uint32_t ip, uint16_t p_src, uint16_t p_dst, const char* msg) { 
    int ml=str_len(msg); uint8_t pl[1024]; UDPHeader* u=(UDPHeader*)pl;
    u->src=hs(p_src); u->dst=hs(p_dst); u->len=hs(8+2+ml); u->chk=0;
    pl[8]='S'; pl[9]=84; 
    for(int k=0;k<ml;k++) pl[10+k]=msg[k]; 
    net_ip(ip, pl, 8+2+ml, 17);
    str_cpy(cmd_status, "UDP: SIGNED (ID 84)");
}

void send_tcp_syn(uint32_t ip, uint16_t port) {
    uint8_t pl[64]; TCPHeader* t=(TCPHeader*)pl;
...
// [REALTEK RX]
void rtl_enable_rx() {
    outl(rtl_io_base + 0x30, (uint32_t)rx_buffer_rtl);
...
void e1000_enable_rx() {
    if(intel_mem_base == 0) return;
    for(int i=0; i<32; i++) {
        rx_bufs_intel[i] = (uint8_t*)(0x800000 + (i * 2048)); 
        rx_desc_intel[i].addr = (uint64_t)(uintptr_t)rx_bufs_intel[i];
        rx_desc_intel[i].status = 0;
    }
    mmio_write32(intel_mem_base + 0x2800, (uint32_t)(uintptr_t)rx_desc_intel); 
...
void e1000_check_rx() {
    if(intel_mem_base == 0) return;
    uint8_t status = rx_desc_intel[rx_cur_intel].status;
    if((status & 1)) {
        str_cpy(cmd_status, "INTEL: PACKET RX!");
        uint8_t* raw_data = rx_bufs_intel[rx_cur_intel];
        uint16_t len = rx_desc_intel[rx_cur_intel].length;
        for(int k=0; k < len - 10; k++) {
            if(raw_data[k] == 'S' && raw_data[k+1] == 84) {
                str_cpy(cmd_status, "MSG (INTEL) DETECTED!");
                char* msg_start = (char*)&raw_data[k+2];
                for(int m=0; m<30; m++) {
                    char c = msg_start[m];
                    if(c >= 32 && c <= 126) cmd_last_out[m] = c;
                    else { cmd_last_out[m] = 0; break; } 
                }
                break;
            }
        }
        rx_desc_intel[rx_cur_intel].status = 0;
        uint32_t old_cur = rx_cur_intel;
...
// UNIVERSAL CHECKER
void check_incoming() {
    if(found_nics[active_nic_idx].type == 2) e1000_check_rx(); 
    else if(found_nics[active_nic_idx].type == 1) { 
        if(rtl_io_base == 0) return;
        uint8_t cmd = inb(rtl_io_base + 0x37);
        if((cmd & 1) == 0) {
            uint32_t* hdr = (uint32_t*)(rx_buffer_rtl + rx_idx_rtl);
            uint32_t rx_stat = hdr[0]; 
            uint32_t rx_len = (rx_stat >> 16) & 0xFFFF;
            if(rx_len == 0 || (rx_stat & 1) == 0) { outw(rtl_io_base + 0x38, rx_idx_rtl - 16); return; }
            char* raw_data = (char*)(rx_buffer_rtl + rx_idx_rtl + 4); 
            for(int k=0; k < rx_len - 10; k++) {
                if(raw_data[k] == 'S' && raw_data[k+1] == 84) {
                    str_cpy(cmd_status, "MSG (RTL) DETECTED!");
                    char* msg_start = &raw_data[k+2];
                    for(int m=0; m<30; m++) {
                        char c = msg_start[m];
                        if(c >= 32 && c <= 126) cmd_last_out[m] = c;
                        else { cmd_last_out[m] = 0; break; } 
            ...
            if(rx_idx_rtl > 8192) rx_idx_rtl = 0; 
            outw(rtl_io_base + 0x38, rx_idx_rtl - 16); 
        }
    }
}

void rtl8139_init(uint32_t io_addr) { 
    rtl_io_base = io_addr & ~3; outb(rtl_io_base + 0x52, 0); outb(rtl_io_base + 0x37, 0x10); 
    while((inb(rtl_io_base + 0x37) & 0x10) != 0) { } 
    outb(rtl_io_base + 0x37, 0x0C); 
    char* p = mac_str; for(int i=0; i<6; i++) { mac_addr[i] = inb(rtl_io_base + i); byte_to_hex(mac_addr[i], p); p+=2; if(i<5) *p++ = ':'; } *p = 0; 
    rtl_enable_rx(); 
    str_cpy(cmd_status, "RTL8139 READY"); str_cpy(ip_address, "DHCP (RTL)..."); 
}
void intel_e1000_init(uint32_t mmio_addr) { 
    intel_mem_base = mmio_addr & 0xFFFFFFF0; 
    if(intel_mem_base > 0) { 
        uint32_t ral = mmio_read32(intel_mem_base + 0x5400); 
        uint32_t rah = mmio_read32(intel_mem_base + 0x5404); 
        mac_addr[0] = (uint8_t)(ral); mac_addr[1] = (uint8_t)(ral >> 8); 
        mac_addr[2] = (uint8_t)(ral >> 16); mac_addr[3] = (uint8_t)(ral >> 24); 
        mac_addr[4] = (uint8_t)(rah); mac_addr[5] = (uint8_t)(rah >> 8); 
        char* p = mac_str; for(int i=0; i<6; i++) { byte_to_hex(mac_addr[i], p); p+=2; if(i<5) *p++ = ':'; } *p = 0; 
...
void uhci_init(uint32_t io_addr) { usb_io_base = io_addr & ~3; outw(usb_io_base, 4); for(int i=0;i<10000;i++); outw(usb_io_base, 0); outw(usb_io_base, 1); if(inb(usb_io_base + 2) & 0) str_cpy(cmd_status, "USB INIT FAIL"); else { str_cpy(cmd_status, "USB HOST RUNNING"); str_cpy(ip_address, "USB SCANNING..."); str_cpy(wifi_ssids[0], "FritzBox (ENC)"); str_cpy(wifi_ssids[1], "Telekom-AB12"); str_cpy(wifi_ssids[2], "Linksys-Home"); str_cpy(wifi_ssids[3], "Open-Wifi"); } }
...
void fs_flush_table() { if(drive_status && drives[0].type == 0) { for(int i=0; i<512; i++) hdd_buf[i] = 0; int off = 0; for(int i=0; i<8; i++) { if(file_table[i].exists) { hdd_buf[off] = 1; char* p = file_table[i].name; for(int k=0;k<30;k++) if(*p) hdd_buf[off+1+k] = *p++; else hdd_buf[off+1+k] = 0; *(uint32_t*)&hdd_buf[off+32] = file_table[i].size; hdd_buf[off+36] = file_table[i].is_folder ? 1 : 0; char* d = file_table[i].date; for(int k=0;k<12;k++) if(*d) hdd_buf[off+37+k] = *d++; else hdd_buf[off+37+k]=0; hdd_buf[off+50] = file_table[i].parent_idx; } off += 64; } ata_write_sector(drives[0].base_port, drives[0].is_slave, 200, hdd_buf); } }
void fs_create_folder(const char* name) { for(int i=0; i<8; i++) { if(!file_table[i].exists) { file_table[i].exists = true; str_cpy(file_table[i].name, name); file_table[i].is_folder = true; file_table[i].size = 0; read_rtc(); char ds[]="00.00.00"; ds[0]='0'+rtc_day/10; ds[1]='0'+rtc_day%10; ds[3]='0'+rtc_mon/10; ds[4]='0'+rtc_mon%10; ds[6]='0'+(rtc_year/10)%10; ds[7]='0'+rtc_year%10; str_cpy(file_table[i].date, ds); file_table[i].parent_idx = current_path_id; fs_flush_table(); return; } } }
void fs_save_file(const char* name, uint32_t size) { if(active_drive_idx != -1 && drives[active_drive_idx].type == 0) { for(int i=0; i<512; i++) hdd_buf[i] = 0; int idx = 0; for(int r=0; r<10; r++) { for(int c=0; c<40; c++) { if(note_buf[r][c]) hdd_buf[idx++] = note_buf[r][c]; else hdd_buf[idx++] = 32; } } ata_write_sector(drives[active_drive_idx].base_port, drives[active_drive_idx].is_slave, 1000, hdd_buf); int slot = -1; for(int i=0; i<8; i++) { if(file_table[i].exists && str_equal(file_table[i].name, name)) { slot = i; break; } } if(slot == -1) { for(int i=0; i<8; i++) { if(!file_table[i].exists) { slot = i; break; } } } if(slot != -1) { file_table[slot].exists = true; str_cpy(file_table[slot].name, name); file_table[slot].is_folder = false; file_table[slot].size = size; file_table[slot].sector_offset = 1000; read_rtc(); char ds[]="00.00.2000"; ds[0]='0'+rtc_day/10; ds[1]='0'+rtc_day%10; ds[3]='0'+rtc_mon/10; ds[4]='0'+rtc_mon%10; ds[6]='0'+(rtc_year/1000)%10; ds[7]='0'+(rtc_year/100)%10; ds[8]='0'+(rtc_year/10)%10; ds[9]='0'+rtc_year%10; str_cpy(file_table[slot].date, ds); file_table[slot].parent_idx = current_path_id; fs_flush_table(); } } }
void fs_save() { fs_save_file("QUICK.TXT", 512); } 
void fs_init() { pci_scan_all(); drive_status = 0; if(ata_probe(ata_base)) drive_status = 1; else if(ata_probe(0x1F0)) drive_status = 1; else if(ata_probe(0x170)) drive_status = 1; ata_scan_drives(); if(drive_count > 0) mbr_scan(); bool found_table = false; if(drive_status && drives[0].type == 0) { ata_read_sector(drives[0].base_port, drives[0].is_slave, 200, hdd_buf); if(hdd_buf[0] != 0) { found_table = true; int off = 0; for(int i=0; i<8; i++) { if(hdd_buf[off] == 1) { file_table[i].exists = true; char* p = file_table[i].name; for(int k=0;k<30;k++) *p++ = hdd_buf[off+1+k]; *p=0; file_table[i].size = *(uint32_t*)&hdd_buf[off+32]; file_table[i].is_folder = (hdd_buf[off+36] == 1); char* d = file_table[i].date; for(int k=0;k<12;k++) *d++ = hdd_buf[off+37+k]; *d=0; file_table[i].parent_idx = hdd_buf[off+50]; } else { file_table[i].exists = false; } off += 64; } } } if(!found_table) { for(int i=0; i<8; i++) file_table[i].exists = false; file_table[0].exists=true; str_cpy(file_table[0].name, "boot.sys"); file_table[0].is_folder=false; file_table[0].size=512; str_cpy(file_table[0].date, "01.01.2023"); file_table[0].parent_idx = 255; file_table[1].exists=true; str_cpy(file_table[1].name, "kernel.bin"); file_table[1].is_folder=false; file_table[1].size=10240; str_cpy(file_table[1].date, "22.05.2023"); file_table[1].parent_idx = 255; } str_cpy(wifi_ssids[0], "Home-WiFi (90%)"); str_cpy(wifi_ssids[1], "Telekom-AB12"); str_cpy(wifi_ssids[2], "Office-Net"); str_cpy(wifi_ssids[3], "Guest-Access"); }

void focus_window(int id) { int found_at = -1; for(int i=0; i<11; i++) if(win_z[i] == id) found_at = i; if(found_at == -1) return; for(int i=found_at; i<10; i++) win_z[i] = win_z[i+1]; win_z[10] = id; }

// ==========================================
// 7. HOLYSPIRIT LOGIC
// ==========================================
void spirit_log_add(const char* msg) {
    for(int i=5; i>0; i--) str_cpy(spirit_logs[i], spirit_logs[i-1]);
...
void spirit_scan() {
    ...
    if(random() % 50 == 0) {
        if(spirit_count < 10) {
            char ip[20]; char b1[4]; int_to_str(192, b1); 
            str_cpy(ip, "192.168.0."); 
            char last[4]; int_to_str(random()%255, last);
            int l = str_len(ip); char* p = last; while(*p) ip[l++] = *p++; ip[l] = 0;
            str_cpy(spirit_targets[spirit_count].ip, ip);
            spirit_targets[spirit_count].hits = 1;
            spirit_targets[spirit_count].critical = false;
...
extern "C" void kernel_main(uint32_t magic, multiboot_info* mbi) {
    // Frühestmögliche Initialisierung von BSS (falls nötig)
    
    // Magic prüfen
    if (magic != 0x2BADB002) {
        // Nicht von GRUB geladen!
        vga_clear();
        vga_print("INVALID BOOTLOADER: ", 0x0C);
        vga_hex(magic);
        while (1) asm("hlt");
    }
    ...
    if (!init_framebuffer(mbi)) {
        vga_clear();
        vga_print("COSMOS v38.5\n", 0x0F);
        vga_print("==============\n", 0x0F);
        vga_print("MBI Flags: ", 0x0E);
        vga_hex(mbi->flags);
        vga_print("\n");
        
        if (mbi->flags & (1 << 12)) {
            vga_print("FB Addr: ", 0x0E);
            vga_hex((uint32_t)mbi->framebuffer_addr);
            vga_print("\nFB Type: ", 0x0E);
            vga_hex(mbi->framebuffer_type);
        } else {
            vga_print("NO FRAMEBUFFER INFO!\n", 0x0C);
            vga_print("Add 'video' to GRUB config.\n", 0x0C);
        }
        
        // Warte auf Taste oder halt
        while (1) asm("hlt");
    }
    
    // Erfolg! Grafikmodus aktiv
    // Optional: Kurze VGA-Bestätigung vor Umschaltung
    // vga_print("GFX OK, switching...\n");
    
    // === ORIGINAL COSMOS INITIALISIERUNG ===
    init_mouse();
    get_cpu_brand();
    fs_init();
    startup_melody();;
    ...
    int clock_dirs[] = {213, 0, 42, 85, 128}; for(int i=0;i<5;i++){ planets[i].ang = clock_dirs[i]; planets[i].dist = 10; planets[i].cur_x = 400; planets[i].cur_y = 300; str_cpy(planets[i].name, (i==0?"TXT":i==1?"APP":i==2?"SYS":i==3?"DSK":"CMD")); }
    for(int i=0;i<200;i++) stars[i]={(i*19)%799-399, (i*29)%599-299, (i*13)+1};
    int cx = 400; int cy = 300; bool is_modal_blocked = false; bool click_consumed = false; 

    while(1) {
        handle_input(); Clear();
        int map_ids[]={0,1,3,4,5}; if(input_cooldown > 0) input_cooldown--; 
        is_modal_blocked = (windows[2].open && !windows[2].minimized);
        z_blocked = false; click_consumed = false; bool mouse_handled = false;
        
        if(frame % 10 == 0) check_incoming();
        ...
        if(active_mode == 0) { 
            ...
            if(galaxy_open && galaxy_expansion < 320) galaxy_expansion += 8; if(!galaxy_open && galaxy_expansion > 0) galaxy_expansion -= 10;
            ...
            if(frame % 200 == 0) read_rtc(); char ts[]="00:00"; ts[0]='0'+rtc_h/10; ts[1]='0'+rtc_h%10; ts[3]='0'+rtc_m/10; ts[4]='0'+rtc_m%10; TextC(v_cx, v_cy+5, ts, 0x000000, true); char ds[]="00.00.2000"; ds[0]='0'+rtc_day/10; ds[1]='0'+rtc_day%10; ds[3]='0'+rtc_mon/10; ds[4]='0'+rtc_mon%10; ds[8]='0'+(rtc_year%100)/10; ds[9]='0'+rtc_year%10; TextC(v_cx, v_cy+20, ds, 0x000000, true);
            ...
            for(int i=0; i<11; i++) {
                ...
                Text(wx+15, wy+15, win->title, (win->id == 10) ? 0x00FF00 : 0x000000, true);
                int bx = wx + ww/2; Text(bx-60, wy+15, "MIN", 0x555555, true); Text(bx-10, wy+15, "FULL", 0x555555, true); Text(bx+50, wy+15, "X", 0x000000, true);
                
                if(win->id == 1) { 
                    TextC(wx+ww/2, wy+40, "INSTALLED APPS", 0x222222, true); 
                    DrawIcon(wx+ww/2-15, wy+80, 0xFFD700); TextC(wx+ww/2, wy+135, "GENESIS DB", 0x333333, true); 
                    if(mouse_just_pressed && !blocked && is_over(mouse_x, mouse_y, wx+ww/2-15, wy+80, 40)) { windows[6].open=true; windows[6].minimized=false; focus_window(6); } 
                    
                    DrawRoundedRect(wx+ww/2-50, wy+180, 100, 30, 5, 0xAAAAAA); TextC(wx+ww/2, wy+188, "ACCESS", 0xFFFFFF, true); 
                    if(mouse_just_pressed && !blocked && is_over_rect(mouse_x, mouse_y, wx+ww/2-50, wy+180, 100, 30)) { windows[8].open=true; windows[8].minimized=false; focus_window(8); } 
                    
                    // --- HOLYSPIRIT LAUNCHER ---
                    DrawIcon(wx+ww/2-15, wy+220, 0xFF00FF);
                    TextC(wx+ww/2, wy+260, "HOLY SPIRIT", 0xFF00FF, true);
                    if(mouse_just_pressed && !blocked && is_over(mouse_x, mouse_y, wx+ww/2-15, wy+220, 40)) { 
                        windows[10].open=true; windows[10].minimized=false; focus_window(10); 
                    }
                }
                ...
                if(win->id == 10) {
                    ...
                    TextC(wx+70, by+8, spirit_active ? "ACTIVE" : "PAUSED", 0xFFFFFF, true);
                    
                    if(mouse_just_pressed && !blocked && is_over_rect(mouse_x, mouse_y, wx+20, by, 100, 30)) {
                        spirit_active = !spirit_active;
                        spirit_log_add(spirit_active ? "SYSTEM ARMED" : "SYSTEM HALTED");
                    }
                    if(spirit_active) {
                        spirit_timer++;
                        if(spirit_timer > 50) { spirit_scan(); spirit_timer=0; }
                    }
                }
                ...
                if(win->id == 0) { 
                    if(key_new && win_z[10] == 0) { if(last_key < 58) { char c = get_ascii(last_key); if(last_key == 0x1C && note_r<9){note_r++; note_c=0;} else if(last_key == 0x0E) { if(note_c>0) {note_c--; note_buf[note_r][note_c]=0;} else if(note_r>0) { note_r--; note_c=39; } } else if(c) { note_buf[note_r][note_c]=c; note_c++; if(note_c > 38) { note_c=0; if(note_r<9) note_r++; } } } }
                    for(int r=0;r<10;r++) Text(wx+20, wy+50+r*15, note_buf[r], 0x333333, true);
                    if((frame/20)%2 == 0 && win_z[10] == 0) DrawChar(wx+20+(note_c*7), wy+50+note_r*15, '_', 0x111111, true);
                ...
                if(win->id == 2) {
                    if(save_step == 0) { 
                        ...
                        if(key_new && win_z[10] == 2) { if(last_key == 0x0E && save_name_idx > 0) save_filename[--save_name_idx] = 0; else if(save_name_idx < 30) { char c = get_ascii(last_key); if(c >= 32) { save_filename[save_name_idx++] = c; save_filename[save_name_idx]=0; } } }
                        ...
                        if(key_new && win_z[10] == 2) { if(last_key == 0x0E && folder_name_idx > 0) new_folder_name[--folder_name_idx] = 0; else if(folder_name_idx < 30) { char c = get_ascii(last_key); if(c >= 32) { new_folder_name[folder_name_idx++] = c; new_folder_name[folder_name_idx]=0; } } }
                ...
                if(win->id == 6) { 
                    int mid=wx+ww/2; Text(wx+20, wy+40, "[ARCH]", genesis_tab == 0?0x000000:0x555555, true); Text(wx+90, wy+40, "[HARD]", genesis_tab == 1?0x000000:0x555555, true); Text(wx+160, wy+40, "[CODE]", genesis_tab == 2?0x000000:0x555555, true);
                ...
                if(win->id == 7) {
                    ...
                    if(key_new && win_z[10] == 7 && wifi_state == 2) { 
                    ...
                    if(wifi_state == 0) { TextC(wx+ww/2, wy+100, "SCANNING AIR...", 0xAAAAAA, true); if(frame % 50 == 0) wifi_state = 1; } 
                        ...
                        if(mouse_just_pressed && !blocked && hov) { wifi_state = 3; send_udp(0xFFFFFFFF, 5000, 80, "COSMOS HELLO"); } 
                    }
