...
extern "C" {
    // Grafik & UI
    void DrawIcon(int x, int y, uint32_t color);
    void DrawRoundWindow(int x, int y, int r, uint32_t color);
    void DrawCentrifugalVortex(int x, int y, int expansion);
    bool is_over(int mx, int my, int x, int y, int r);
    ...
    // Netzwerk
    void send_udp(uint32_t ip, uint16_t src_port, uint16_t dest_port, const char* msg);
    void send_tcp_syn(uint32_t ip, uint16_t port);
}
// --- PROTOTYPEN (Damit der Compiler die Funktionen kennt) ---
extern "C" {
    ...
    void DrawCentrifugalVortex(int x, int y, int expansion);
    void system_reboot();
    void play_earthquake(int duration);
}
void str_cpy(char* dest, const char* src) {
    while (*src) { *dest++ = *src++; }
    *dest = '\0';
...
// --- Fehlende Grafik-Funktionen ---
void DrawRoundWindow(int x, int y, int r, uint32_t color) {
    for (int dy = -r; dy <= r; dy++) {
        for (int dx = -r; dx <= r; dx++) {
            if (dx * dx + dy * dy <= r * r) {
                // Hier dein Framebuffer-Schreibzugriff, z.B.:
...
void DrawIcon(int x, int y, uint32_t color) {
    // Minimales 8x8 Quadrat als Platzhalter für Icons
    for(int i=0; i<8; i++) 
        for(int j=0; j<8; j++) /* put_pixel(x+i, y+j, color); */ ;
}

void DrawCentrifugalVortex(int x, int y, int exp) {
    // Platzhalter für den Vortex-Effekt
}

// --- Fehlende Eingabe-Funktionen ---
char get_ascii(uint8_t scancode) {
    // Minimales Mapping für Buchstaben (US-Layout Beispiel)
    static const char map[] = "??1234567890-+=??qwertyuiop[]??asdfghjkl;'`?\\zxcvbnm,./";
    if (scancode < 58) return map[scancode];
    return 0;
}

// --- Fehlende Netzwerk-Stubs (Damit es linkt) ---
void send_udp(uint32_t ip, uint16_t src, uint16_t dest, const char* msg) { /* Später implementieren */ }
void send_tcp_syn(uint32_t ip, uint16_t port) { /* Später implementieren */ }	
...
// 1. KEIN static! Die Struktur muss global sein.
...
extern "C" void kernel_main(void) {
    // 1. Check, ob Limine geantwortet hat
    if (fb_req.response == nullptr || fb_req.response->framebuffer_count < 1) {
        while(1) { __asm__("hlt"); }
    }

    // 2. Variable EINMAL deklarieren
    struct limine_framebuffer *lfb = fb_req.response->framebuffers[0];

    // 3. Pointer setzen
    fb = (uint32_t*)lfb->address;
    bb = fb; // Erstmal direkt auf den Schirm schreiben
    ...
    // 4. TEST-BALKEN (Grün)
    // Wir zeichnen direkt, um zu sehen, ob wir "leben"
    for (uint32_t i = 0; i < (screen_w * 100); i++) {
        fb[i] = 0x00FF00; 
    }
